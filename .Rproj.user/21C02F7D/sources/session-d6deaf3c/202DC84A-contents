...
#' @title MAC statistic.
#' @description Compute the MAC statistic between two data sets (X, Y) and (X2, Y2).
#' @param X X is the predictors in the first data set and can be an n times p dimensional matrix, where n is the number of the points.
#' @param Y Y is the response in the first data set. It can be a vector of length n or an n times 1 dimensional matrix.
#' @param X2 X2 is the predictors in the second data set and can be an n times p dimensional matrix.
#' @param Y2 Y2 is the response in the second data set. The data type of Y2 should be consistent with Y.
...
#' @title Structure improvement evaluation for continuous Y.
#' @description Compute the structure improvement index p_s for a continuous response Y.
...
#' @param cores The number of threads used for parallel computing. It can only be 1 in Windows environment
...
ps_continuous <- function(X, Y, Y2, cores = 1, BootNumber = 1000) {
...
#' @title Structure improvement evaluation based on predicted probability.
#' @description Compute the structure improvement index p_s based on predicted probability. When Y is a binary variable,
#'     we use the probability vector given by the trained model to calculate the model structure improvement.
#' @param X The predictors in the training set, which is an n times p dimensional matrix.
#' @param Y The true response in the training set, which is an n-dimensional vector.
#' @param Y2 The predicted probability of the trained model on the training set X.
#' @param cores The number of threads used for parallel computing. It can only be 1 in Windows environment
#' @param BootNumber The number of bootstrap steps in the calculation of p_s.
#'
#' @returns The structure improvement index p_s. If p_s < 0.05, the model fit is not good enough and we need to increase the complexity of the model.
